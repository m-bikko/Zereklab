...
export default function BonusManagement() {
  ...
  const handleCreateNewCustomer = async () => {
    ...
    setSubmitting(true);
    try {
      const response = await fetch(`/api/bonuses?phone=${encodeURIComponent(newPhoneNumber)}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const totalBonusesIssued = bonuses.reduce((sum, bonus) => sum + bonus.totalBonuses, 0);
  const totalBonusesUsed = bonuses.reduce((sum, bonus) => sum + bonus.usedBonuses, 0);
  const totalAvailableBonuses = bonuses.reduce((sum, bonus) => sum + bonus.availableBonuses, 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Управление бонусами</h2>
          <p className="text-gray-600">Просмотр и управление бонусной системой клиентов</p>
        </div>
                  ...
                  <tr key={bonus._id} className="hover:bg-gray-50">
                    ...
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                      <div className="flex items-center">
                        ...
                        {new Date(bonus.lastUpdated).toLocaleDateString('ru-RU')}
                      </div>
                    </td>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="w-full max-w-md rounded-lg bg-white p-6">
            <h3 className="mb-4 text-lg font-medium">Добавить нового клиента</h3>
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setNewPhoneNumber(cleanPhoneInput(e.target.value))}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="number"
                  min="1"
                  max={operationType === 'deduct' ? selectedBonus.availableBonuses : undefined}
              ...
              <div className="flex justify-end space-x-3">
                ...
                >
                  {submitting ? (
                    ...
                    <span>{operationType === 'add' ? 'Начислить' : 'Списать'}</span>
                  )}
                </button>
              </div>
            </div>
