...
export default function SalesPage() {
  ...
  const searchBonuses = async () => {
    ...
    try {
      ...
      const [bonusResponse, pendingResponse] = await Promise.all([
        ...
        fetch(
          `/api/pending-bonuses?phone=${encodeURIComponent(customerPhone)}`
        ),
      ]);
  ...
  const processSale = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success(
          `Продажа оформлена! ${result.sale.bonusesEarned} бонусов будут доступны ${new Date(result.sale.bonusAvailableDate).toLocaleDateString('ru-RU')}`
        );
  ...
  const filteredProducts = products.filter(
    product =>
      getLocalizedText(product.name, 'ru')
        .toLowerCase()
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="mx-auto max-w-6xl">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600">
                Оформление продаж и управление бонусами
              </p>
            </div>
          ...
          <>
                          ...
                          >
                            ...
                            <div className="flex-1">
                              <h4 className="font-medium text-gray-900">
                                {getLocalizedText(item.product.name, 'ru')}
                              </h4>
                ...
                <div className="xl:col-span-1">
                  <div className="rounded-lg bg-white p-6 shadow-sm">
                        ...
                        <div className="border-t pt-3">
                          ...
                          {bonusesToUse === 0 ? (
                            <button
                              ...
                              className="flex w-full items-center justify-center space-x-2 rounded-lg bg-purple-500 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-purple-600"
                            >
                              <Gift className="h-4 w-4" />
                            ...
                            <div className="space-y-3">
                              <div className="flex justify-between text-sm">
                                ...
                                <span className="font-medium text-purple-600">
                                  -{formatPrice(bonusesToUse)}
                                </span>
                              </div>
                              <button
                                onClick={() => setBonusesToUse(0)}
                                className="w-full rounded-lg border border-purple-300 bg-white px-4 py-2 text-sm font-medium text-purple-600 transition-colors hover:bg-purple-50"
                              >
                    ...
                    <button
                      onClick={processSale}
                      disabled={
                        processing || !isValidPhoneNumber(customerPhone)
                      }
            ...
            {showProductModal && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                <div className="m-4 max-h-[80vh] w-full max-w-4xl overflow-hidden rounded-lg bg-white">
            ...
            {showBonusModal && bonusData && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                <div className="m-4 max-h-[80vh] w-full max-w-md overflow-hidden rounded-lg bg-white">
                  <div className="flex items-center justify-between border-b bg-white p-4">
                    <h2 className="text-xl font-semibold text-gray-900">
                      Бонусы клиента
                    </h2>
                    ...
                    <div className="space-y-4">
                      ...
                      {bonusData.pendingBonuses && (
                        <div className="space-y-3">
                          ...
                          {bonusData.pendingBonuses.available &&
                            bonusData.pendingBonuses.available.length > 0 && (
                              <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-4">
                                ...
                                <div className="space-y-2">
                                  {bonusData.pendingBonuses.available.map(
                                    bonus => (
                                      <div
                                        key={bonus._id}
                          ...
                          {/* Upcoming bonuses */}
                          {bonusData.pendingBonuses.upcoming &&
                            bonusData.pendingBonuses.upcoming.length > 0 && (
                              <div className="rounded-lg border border-orange-200 bg-orange-50 p-4">
                                ...
                                <div className="space-y-2">
                                  {bonusData.pendingBonuses.upcoming.map(
                                    bonus => (
                                      <div
                                        key={bonus._id}
                      ...
                      <div className="text-center">
                        <p className="text-xs text-gray-500">
                          ...
                          {new Date(bonusData.lastUpdated).toLocaleDateString(
                            'ru-RU'
                          )}
                        </p>
                      </div>
        ...
        {activeTab === 'processing' && <BonusProcessing />}
      </div>
    </div>
  );
}
